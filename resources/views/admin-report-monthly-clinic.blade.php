...
<head>
    ...
    <title>Monthly Clinic Report - Khayra Physio</title>
...
<body>
@php $activeReport = 'monthly'; @endphp
<div class="layout">
        ...
        <div class="container">
            <div class="topbar">
                <div>
                    ...
                    <p class="subtitle">Ringkasan bulanan untuk owner: booking, visit, pasien baru, revenue, outstanding, produk keluar, movement stok, dan void.</p>
                </div>
                <div class="actions">
                    <a href="/admin/reports/monthly-clinic?month={{ $month }}&export=csv" class="btn btn-primary">Export CSV</a>
...
<div class="nav-grid">
    ...
    <a class="nav-card {{ ($activeReport ?? '') === 'therapist' ? 'active' : '' }}" href="/admin/reports/therapist-performance">Therapist Performance</a>
            ...
            <section class="hero">
                <div class="hero-grid">
                    <div class="hero-main">
                        <span class="badge">{{ $summary['month_label'] }}</span>
                        <h2 class="section-title">Owner snapshot untuk kondisi klinik bulan ini.</h2>
                        <p class="section-subtitle">Laporan ini menggabungkan data appointment, visit, billing, promo/discount, inventory usage, stock movement, dan invoice void.</p>
                    </div>
                    <aside class="hero-side">
                        <h3>Executive summary</h3>
                        <p>
                            Paid amount bulan ini Rp {{ number_format($summary['paid_amount'], 0, ',', '.') }},
                            outstanding Rp {{ number_format($summary['outstanding'], 0, ',', '.') }},
                            dengan {{ $summary['void_invoices'] }} invoice void.
                        </p>
                    </aside>
                </div>
            </section>
            ...
            <section class="filter-card">
                <form method="GET" action="/admin/reports/monthly-clinic" class="filter-form">
                    <div class="field">
                        <label>Month</label>
            ...
            <section class="stats-grid five">
                <div class="stat-card blue"><div class="stat-label">Booking</div><div class="stat-value">{{ $summary['bookings'] }}</div><div class="stat-sub">Total appointment bulan ini.</div></div>
                <div class="stat-card green"><div class="stat-label">Visit</div><div class="stat-value">{{ $summary['visits'] }}</div><div class="stat-sub">{{ $summary['completed_visits'] }} completed.</div></div>
                <div class="stat-card violet"><div class="stat-label">New Patients</div><div class="stat-value">{{ $summary['new_patients'] }}</div><div class="stat-sub">Pasien baru bulan ini.</div></div>
                ...
                <div class="stat-card orange"><div class="stat-label">Outstanding</div><div class="stat-value">Rp {{ number_format($summary['outstanding'], 0, ',', '.') }}</div><div class="stat-sub">Unpaid + partial.</div></div>
            </section>

            <section class="stats-grid">
                <div class="stat-card blue"><div class="stat-label">Gross Revenue</div><div class="stat-value">Rp {{ number_format($summary['gross_revenue'], 0, ',', '.') }}</div><div class="stat-sub">Sebelum diskon.</div></div>
                <div class="stat-card red"><div class="stat-label">Discount</div><div class="stat-value">Rp {{ number_format($summary['discount'], 0, ',', '.') }}</div><div class="stat-sub">Total promo / diskon.</div></div>
                <div class="stat-card green"><div class="stat-label">Net Revenue</div><div class="stat-value">Rp {{ number_format($summary['net_revenue'], 0, ',', '.') }}</div><div class="stat-sub">Setelah diskon, exclude void.</div></div>
                <div class="stat-card red"><div class="stat-label">Void Invoice</div><div class="stat-value">{{ $summary['void_invoices'] }}</div><div class="stat-sub">Invoice dibatalkan.</div></div>
            </section>

            <div class="two-grid">
                <section class="section-card">
                    <h2 class="section-title">Inventory Movement</h2>
                    <p class="section-subtitle">Ringkasan stok masuk, keluar, dan adjustment.</p>
                    <div class="stats-grid" style="grid-template-columns:repeat(3,1fr); margin:0;">
                        <div class="stat-card green"><div class="stat-label">Stock In</div><div class="stat-value">{{ $summary['stock_in'] }}</div></div>
                        <div class="stat-card red"><div class="stat-label">Stock Out</div><div class="stat-value">{{ $summary['stock_out'] }}</div></div>
                        <div class="stat-card blue"><div class="stat-label">Adjustments</div><div class="stat-value">{{ $summary['adjustments'] }}</div></div>
                    </div>
                </section>

                <section class="section-card">
                    <h2 class="section-title">Top Product Usage</h2>
                    <p class="section-subtitle">Produk inventory paling banyak keluar lewat invoice valid.</p>
                            ...
                            <table style="min-width:600px;">
                                ...
                                <tbody>
                                    @foreach($topProducts->take(5) as $product)
                                        <tr>
                                            <td><div class="primary-text">{{ $product['name'] }}</div><div class="secondary-text">{{ $product['lines'] }} line invoice</div></td>
                                            <td>{{ $product['qty'] }}</td>
                                            <td class="money">Rp {{ number_format($product['value'], 0, ',', '.') }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
                    ...
                    @else
                        <div class="empty-state">Belum ada product usage pada periode ini.</div>
                    @endif
                </section>
            </div>
            ...
            <section class="section-card">
                ...
                <p class="section-subtitle">Breakdown harian untuk melihat tanggal yang ramai, menghasilkan revenue, atau punya outstanding.</p>
                <div class="table-wrap">
                    <table>
                        <thead>
                            <tr>
                        ...
                        <tbody>
                            @foreach($dailyRows as $row)
                                <tr>
                                    <td><div class="primary-text">{{ $row['date'] }}</div></td>
                                    <td>{{ $row['bookings'] }}</td>
                                    <td>{{ $row['visits'] }}</td>
                                    <td>{{ $row['new_patients'] }}</td>
                                    <td class="money">Rp {{ number_format(($row['paid_amount'] ?? $row['paid'] ?? 0), 0, ',', '.') }}</td>
                                    <td class="money">Rp {{ number_format(($row['outstanding_amount'] ?? $row['outstanding'] ?? 0), 0, ',', '.') }}</td>
                                    <td>{{ $row['void'] }}</td>
                                </tr>
                            @endforeach
                        </tbody>
