...
<head>
    ...
    <style>
        *{box-sizing:border-box}
        ...
        .box{border:1px solid #eef2f1;background:#fbfcfc;border-radius:14px;padding:14px 16px;white-space:pre-line;line-height:1.9;font-size:14px;color:#374151}
...
<div class="page">
    ...
    <div class="sheet">
        ...
        <div class="number">Tanggal: {{ \Carbon\Carbon::parse($payload['summary_date'])->format('Y-m-d') }}</div>
        ...
        <div class="meta">
            ...
            <div class="meta-label">Visit Terakhir</div><div>{{ $visit && $visit->visit_date ? $visit->visit_date : '-' }}</div>
        ...
        <div class="section">
            ...
            <div class="box">{{ $payload['initial_condition'] ?: ($record ? ($record->complaint ?: $record->condition_felt) : '-') ?: '-' }}</div>
        </div>

        <div class="section">
            <div class="section-title">Kondisi Akhir / Progress</div>
            <div class="box">{{ $payload['final_condition'] ?: ($record ? ($record->response_to_treatment ?: $record->progress_note) : '-') ?: '-' }}</div>
        </div>

        <div class="section">
            <div class="section-title">Ringkasan Terapi</div>
            <div class="box">{{ $payload['therapy_summary'] ?: ($record ? ($record->treatment_given ?: $record->treatment) : '-') ?: '-' }}</div>
        </div>

        <div class="section">
            <div class="section-title">Home Program</div>
            <div class="box">{{ $payload['home_program'] ?: '-' }}</div>
        </div>

        <div class="section">
            <div class="section-title">Rekomendasi Lanjutan</div>
            <div class="box">{{ $payload['recommendation'] ?: ($record ? ($record->recommendation ?: $record->next_session_plan ?: $record->control_plan) : '-') ?: '-' }}</div>
        </div>
